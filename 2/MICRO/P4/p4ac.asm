...
CODE SEGMENT 
	...
	;_______________________Rutina de servicio a la interrupcion 1Ch_______________________
	; En ella comprobaremos que pasa un segundo en funcion de la periocididad en la llamada
	; a esta interrupcion. 
	;
	; Comprobamos el valor de al porque, como esta interrupcion la llama el
	; sistema siempre, solo queremos hacer cosas cuando ya tengamos una cadena lista, y es entonces
	; cuando al se actualizara.
	;
	; 
	;______________________________________________________________________________________
	rsi2 PROC FAR
		...
		mov aux, al	;guardamos el valor inicial de al
		
		mov al, cont	
		inc al
		mov cont, al
		
		cmp al, 18	;Imprimiremos cada segundo
		jne acaba
		mov cont, 0	;Reseteamos el contador de segundos para que vuelva a empezar la cuenta
	...
	; Rutina que recibe una cadena en DS:DX y muestra la codificacion del caracter apuntado
	; por si en Polibio.
	; Actualiza el valor de al en el caso de que se haya procesado toda la cadena
	; 
	;______________________________________________________________________________________
	COD PROC NEAR
		push bp
	...
	codFinal:
		mov al, 0EFh ;indicamos que hemos acabado
		jmp vete
		
	final2:	
		mov al, aux	;restauramos el valor con el que al venia
		
	vete:
		inc si	;incrementamos el indice para la siguiente
	...
	; Realiza el proceso inverso al de la funcion de codificacion
	;______________________________________________________________________________________
	DECOD PROC NEAR
		push bp
		mov bp, sp
	...
	decodNumeroPequeno:
		...
		;Si es un numero pequeño el numero es la columna -2
		dec bl 
		dec bl 
		mov al, bl
		add al, 65
	...
	;______________________________________________________________________________________
	;_______________________Procedimiento de instalacion de rsi para la 1Ch________________
	; Vamos a instalarla siempre ya que por defecto viene instalado algo que solo hace iret,
	; entonces no podemos comprobar si ya hemos instalado previamente
	;______________________________________________________________________________________
	instalador PROC NEAR
		...
		mov ax, offset rsi2 ;offset donde esta la RSI
		mov bx, cs ;segmento en donde esta la RSI
		
		cli
		mov es:[1Ch*4], ax ;offset de la rsi
		mov es:[1Ch*4+2], bx ;segmento de la rsi
		sti
	...
	;______________________________________________________________________________________
	;_______________________Procedimiento de desinstalacion de rsi de 1Ch_________________________
	;Permite liberar la memoria que se dejo residente, desinstalando la RSI de la int. 1Ch
	;______________________________________________________________________________________
	desinstalador PROC NEAR
		...
		mov es, ds:[1Ch*4+2] ;segmento de la rsi
		mov bx, es
		...
		cli
		mov ds:[1Ch*4], cx
		mov ds:[1Ch*4+2], cx ;pone a 0 el vector de interrupcion de la 57h
		sti
	...
	mostrarEstado PROC NEAR
		...
		cmp es:[1Ch*4+2], ax ;comprobar estado del driver
		jne driverSi
		jmp driverNo
